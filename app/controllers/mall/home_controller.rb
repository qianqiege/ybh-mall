class Mall::HomeController < Mall::BaseController
  def index
    ...
    @products = @q.result(distinct: true).select("name,general,now_product_price,original_product_price,is_test,priority,display,id").where(display: true).order("priority DESC, id DESC")
    if !params[:id].nil?
