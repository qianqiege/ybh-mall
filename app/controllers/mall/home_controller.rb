class Mall::HomeController < Mall::BaseController
  ...
  def index
    ...
    @products = @q.result(distinct: true).select("image,name,general,now_product_price,original_product_price,is_test,priority,display,id,contents_category_id").where(display: true).order("priority DESC, id DESC")
    @id = params[:id]
    # 判断用户不是测试人员时， 筛选非测试产品
  ...
  def class_detail 
    @slides = Slide.top(2)
    @q = Product.ransack(params[:q])
    @products = @q.result(distinct: true).select("image,name,general,now_product_price,original_product_price,is_test,priority,display,is_pendding_sale,id").where(display: true).order("priority DESC, id DESC")
    if !params[:id].nil?
