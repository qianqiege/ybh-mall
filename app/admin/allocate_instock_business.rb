ActiveAdmin.register AllocateInstockBusiness do

  menu parent: I18n.t("active_admin.menu.spd_allocate"), priority: 3
...
#form
  form do |spd_business|
    spd_business.inputs do
      number = AllocateInstockBusiness.generator_number params
...
#show
  show do
    ...
    panel "订单项详情" do
      table_for allocate_instock_business.spd_business_items do |item|
  ...
  member_action :review, method: [:get, :post] do
    if resource.may_review?
      resource.review
      resource.save
      redirect_to admin_allocate_instock_businesses_path
    else
      redirect_to :back, notice: "只能审核，等待审核的订单!"
    end
  end
  action_item :super_action, only: :show do
    link_to "调拨入库审核", review_admin_allocate_instock_business_path
  end
