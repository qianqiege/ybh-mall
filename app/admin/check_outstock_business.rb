ActiveAdmin.register CheckOutstockBusiness do

  menu parent: I18n.t("active_admin.menu.spd_check"), priority: 12
  ...
  form do |spd_business|
    spd_business.inputs do
      panel "基础信息", id: 'base_info' do
        number = CheckOutstockBusiness.generator_number params
...
#show
  show do
    ...
    panel "订单项详情" do
      table_for check_outstock_business.spd_business_items do |item|
  ...
  member_action :review, method: [:get, :post] do
    if resource.may_review?
      ...
      resource.save
      redirect_to admin_check_outstock_businesses_path
    else
      redirect_to :back, notice: "只能审核，等待审核的订单!"
    end
  end
  action_item :super_action, only: :show do
    link_to "盘点出库审核", review_admin_check_outstock_business_path
  end
